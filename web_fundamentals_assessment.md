...
## BONUS SECTION: Build a Weather App (10 points)

### Create a Command-Line Weather Application

Build a CLI application that fetches weather data from a public API.
...
- Color-coded output based on temperature
- Cache recent requests to avoid API rate limits
...
```python
if __name__ == "__main__":
    print("=== Testing Web Fundamentals Assessment ===\n")
    
    # Section 1 Tests
    print("Section 1: HTTP Fundamentals")
    response = demonstrate_get("https://jsonplaceholder.typicode.com/posts/1")
    ...
    # Section 3 Tests
    print("\nSection 3: JSON Handling")
    stats = calculate_statistics(posts)
    print(f"Statistics: {stats}")
    
    grouped = group_posts_by_user(posts[:20])
    print(f"Posts grouped by {len(grouped)} users")
    
    # Section 4 Tests
    print("\nSection 4: GitHub API")
    gh_client = GitHubClient()
    user = gh_client.get_user("octocat")
    if user:
        print(f"GitHub user: {user['login']}")
    
    repos = gh_client.search_repositories("python", max_results=3)
    print(f"Found {len(repos)} Python repositories")
    
    rate_limit = gh_client.get_rate_limit()
    print(f"Rate limit remaining: {rate_limit['rate']['remaining']}")
...
- **80-100 points**: Excellent! Ready for Databases and FastAPI
- **60-79 points**: Good understanding, practice more with APIs
- **40-59 points**: Decent foundation, review HTTP and REST concepts
- **Below 40 points**: Review web fundamentals before proceeding

---

## Submission Checklist

- [ ] All HTTP methods demonstrated correctly
- [ ] Status codes and headers handled properly
- [ ] JSONPlaceholder client fully implemented
- [ ] Error handling and retries working
- [ ] JSON parsing and validation complete
- [ ] GitHub API client functional (rate limits respected)
- [ ] API design document created
- [ ] Best practices document completed
- [ ] Code is properly commented
- [ ] Test cases pass
- [ ] Bonus weather app (if attempted) is functional
...
- **API Rate Limits:** Be respectful of API rate limits, especially with GitHub (60 requests/hour without auth)
- **API Keys:** Never commit API keys to version control
- **Error Handling:** Always handle network errors gracefully
- **Timeouts:** Always set timeouts for requests
- **Testing:** Test with real APIs but have fallback test data
